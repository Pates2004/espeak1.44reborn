...
static double DBtoLIN (int);
static void frame_init (klatt_frame_ptr); 
static void setabc (int,int,resonator_ptr);
static void setzeroabc (int,int,resonator_ptr);
...
static void frame_init(klatt_frame_ptr frame) 
{
	...
	setabc((int)0.0,(int)(kt_globals.samrate/2),&(kt_globals.rsn[Rout]));
...
//	printf("truncated, nopen = %d\n",kt_globals.nopen);
		...
		setabc((int)0,temp,&(kt_globals.rsn[RGL]));
...
static void setabc(int f, int bw, resonator_ptr rp)
{
	double r;
	double arg;
...
static void setzeroabc(int f, int bw, resonator_ptr rp)
{
	double r;
	double arg;
...
static double gen_noise(double noise) 
{
	...
	temp = (int) getrandom(-8191,8191);
	kt_globals.nrand = (int) temp;
