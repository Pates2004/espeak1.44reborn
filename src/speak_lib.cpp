...
{//=======================================================
	...
	case AUDIO_OUTPUT_PLAYBACK:
		...
		synchronous_mode = 0;
	#else
		// The historical Windows build has no POSIX FIFO worker.  Preserve a
		// functional playback API by running the request synchronously through
		// the native WinMM output layer.
		my_mode = AUDIO_OUTPUT_SYNCH_PLAYBACK;
		option_waveout = 0;
		WavegenInitSound();
	#endif
		break;
...
{//===========================================================================
	...
	SHOW("sync_espeak_Synth > position=%d, position_type=%d, end_position=%d, flags=%d, user_data=%p, text=%s\n", position, position_type, end_position, flags, (void*)user_data, text);
#endif
